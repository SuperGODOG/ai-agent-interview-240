...
这一课覆盖的完整考点，并按面试真实追问的深度重新组织。**IO 模型是 Netty、Redis、Nginx、Kafka 的底层地基** —— 面试问「Java IO 模型」看似基础，但只要你能把 `select` 和 `epoll` 的区别画出来、把 LT/ET 讲清楚、把「同步非阻塞 IO」和「IO 多路复用」的边界说明白，面试官立刻会把你从「背八股」升级到「懂原理」的档位。
