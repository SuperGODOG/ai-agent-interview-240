...
这一课覆盖的核心考点。`CopyOnWriteArrayList`（下文简称 **CoW**）是 *读多写少* 场景下的并发神器 —— 它的思想极简（**写的时候复制一份，改副本；读的时候永远读旧的**），却踩坑无数：*内存翻倍、弱一致性、大集合写入雪崩* 都会被面试官追着问。串起来这一课，也是理解「不可变数据 + 引用替换」这条并发思路的一个绝佳入口。
